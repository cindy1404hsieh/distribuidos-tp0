...
class Server:
    def __init__(self, port, listen_backlog):
        ...
        self.lottery_done = False     # sorteo hecho?
    ...
    def run(self):
        ...
        Ahora maneja timeout de inscripción dinámico.
        """
        start_time = time.time()
        try:
            while self._running:
                # cerrar inscripción si pasó el timeout
                if time.time() - start_time > self.registration_timeout:
                    logging.info("Registration timeout reached, stopping new connections")
                    break
                try:
                    ...
                    if not self._running:
                        break
                    raise
            # después de timeout hacemos el sorteo si al menos 1 agencia llegó
            with self.state_lock:
                if not self.lottery_done and self.known_agencies:
                    self.__do_lottery()
        finally:
            self.__cleanup()
    ...
    def __handle_batch(self, client_sock, msg):
        ...
        try:
            ...
            send_message(client_sock, int_to_bytes(last_number, 4))
            logging.info(f"action: apuesta_recibida | result: success | cantidad: {len(bets)}")
        except Exception as e:
            logging.error(f"action: apuesta_recibida | result: fail | error: {e}")
            try:
                send_message(client_sock, int_to_bytes(0, 4))
            except:
                pass
    ...
    def __handle_done(self, client_sock, msg):
        ...
        try:
            ...
            with self.state_lock:
                ...
                if not self.lottery_done and len(self.known_agencies) == len(self.agencies_done):
                    should_do_lottery = True
            send_message(client_sock, bytes([0x01]))  # ACK
            ...
            with self.lottery_condition:
                while not self.lottery_done and self._running:
                    ...
                    if not self.lottery_condition.wait(timeout=0.5):
                        if not self._running:
                            send_message(client_sock, bytes([MESSAGE_TYPE_NOT_READY]))
                            return
